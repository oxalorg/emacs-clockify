...
(defun clockify-get-projects ()
  (interactive)
  (message "Getting all clockify projects...")
  (setq clockify-projects '())
  (let ((page 1)
        (page-size 5000)
        (break nil))
    (while (not break)
      (message (format "Fetching page: %d" page))
      (let* ((params (format "?page=%d&page-size=%d" page page-size))
             (projects (clockify-api "GET" (concat "/workspaces/" clockify-workspace "/projects" params))))
        (progn
          (message (format "Total projects fetched: %d" (length clockify-projects)))
  ...
  (setq clockify-project-client
        (mapcar (lambda (project)
                ...
                clockify-projects))
  (message "Done. Fetched all clockify data."))
